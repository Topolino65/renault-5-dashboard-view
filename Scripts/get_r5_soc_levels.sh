...
RAW_OUT="/config/renault/debug_soc_levels_raw.txt"
CACHE_JSON="/config/renault/soc_levels_cache.json"
...
URL="/commerce/v1/accounts/${KAM_ACCOUNT_ID}/kamereon/kcm/v1/vehicles/${VIN}/ev/soc-levels"
...
RAW_OUT = Path("/config/renault/debug_soc_levels_raw.txt")
CACHE_JSON = Path("/config/renault/soc_levels_cache.json")
VIN = "<your-vin-number>"
...
def load_cache(err: str, note: str):
    if CACHE_JSON.exists():
        ...
        cached_attrs = cached.get("data", {}).get("attributes", {}) if isinstance(cached, dict) else {}
        cached = wrap_attributes(cached_attrs, False, err, "cache", note)
...
# Prefer JSON first; fall back to literal_eval
try:
    obj = json.loads(blob)
except Exception:
    try:
