...
RAW_OUT="/config/renault/debug_hvac_raw.txt"
CACHE_JSON="/config/renault/hvac_cache.json"
...
URL="/commerce/v1/accounts/${KAM_ACCOUNT_ID}/kamereon/kca/car-adapter/v1/cars/${VIN}/hvac-status?country=GB&brand=RENAULT"
...
RAW_OUT = Path("/config/renault/debug_hvac_raw.txt")
CACHE_JSON = Path("/config/renault/hvac_cache.json")
VIN = "<your-vin-number>"
...
def load_cache_as_ok(note):
    if CACHE_JSON.exists():
        ...
        if isinstance(old_attrs, dict):
            ...
            old_attrs["hvacStatus"] = "unknown"
        print(json.dumps(wrap(old_attrs, True, None, "cache", note)))
        return True
    return False
...
if any(x in raw_low for x in prompt_markers):
    # HVAC cache is misleading; mark as ok but stale with hvacStatus unknown
    if not load_cache_as_ok("stale_cache_used_cli_prompt_detected"):
